...
namespace gem5
{
    ...
    namespace sDM
    {
                                       ...
                                       uint32_t h)
        {
            ...
            uint8_t testbuf[PAGE_SIZE >> 1];
            read4Mem(PAGE_SIZE>>1,testbuf,0x267000);
            // sDMdump((char *)"read memory test in init", testbuf, 32);
            return;
        }
        /**
         * @brief 为数据空间构建sDM空间
        ...
        sDMmanager::sDMspace_register(uint64_t pid, Addr vaddr, size_t data_byte_size)
        {
            ...
            Tick next = curTick() + 50000;
            this->eventq->unlock();
            this->eventq->serviceEvents(next);
            this->eventq->lock();
            // 准备新空间的metadata
            sdm_space sp;
            // 1. 计算data大小
                                     ...
                                     uint8_t *hpg_data, iit_NodePtr counters, sdm_hashKey hash_key)
        {
            ...
            sDMdump((char *)"stored data",hpg_data,16);
            uint8_t testbuf[32];
            read4Mem(32,testbuf,0x267000);
            sDMdump((char *)"0x267000 data",testbuf,16);
            //  计算HMAC
            hmac_get(hpg_data, pageAddr, counters, hash_key, calc_hmac);
            printf("start_2:\n");
            sDMdump((char *)"hmac_verify:counter", (u_int8_t *)counters, 64);
            sDMdump((char *)"Hmac_get:hkey", hash_key, 32);
            printf("Hmac_get:dataptr:%8lx,hkey:%8lx,paddr:%lx,counter:%lx,",(uint8_t *)*hpg_data, *hash_key, pageAddr, (uint8_t *)counters);
            // 与存储值比较
            int pnum = 0;
            // printf("rva=%lx\n", rva);
            // rva /= SDM_HMAC_ZOOM;    
        ...
        void sDMmanager::read(PacketPtr pkt, uint8_t *algined_mem_ptr, Addr pkt_vaddr)
        {
            ...
            sDMdump((char *)"read_verify:hkey", hash_key, 16);
            ...
            sDMdump((char *)"decrypted data", algined_mem_ptr, 16);
            printf("read succeed\n");
        }
        /**
         * @author yqy
        ...
        sDMmanager::write(PacketPtr pkt, uint8_t *aligned_mem_ptr, Addr pktVAddr)
        {
            ...
            printf("check write[%lx:%lx]\n", pktVAddr, pktVAddr + CL_SIZE - 1);
            int h;
            Addr keyPathAddr[MAX_HEIGHT] = {0};
            iit_Node keyPathNode[MAX_HEIGHT] = {0};
            sdm_hashKey hash_key;
